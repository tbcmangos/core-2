...
struct FactionEntry
{
    ...
    //char*       name[16];                                   // 23-38    m_name_lang cmangos
    std::string   name[16];                                               //vmangos
    // 39 string flags
...
#if SUPPORTED_CLIENT_BUILD > CLIENT_BUILD_1_12_1
//not in use, vmangos are using the map_template inside the database
/*struct MapEntry
{
    uint32  MapID;                                          // 0        m_ID
...
#if SUPPORTED_CLIENT_BUILD > CLIENT_BUILD_1_12_1
//already defined in vmangos ObjectMgr.h
...
//not currently in use due to vmangos using spell_template inside database
/*struct SpellEntry
{
        uint32    Id;                                       // 0        m_ID
...
struct TaxiNodesEntry
{
    ...
    float     z;                                            // 4        m_z
#if SUPPORTED_CLIENT_BUILD > CLIENT_BUILD_1_12_1
    //char*     name[16];                                     // 5-21     m_Name_lang cmangs
    std::array<std::string, MAX_DBC_LOCALE> name{};         // 5-12     m_Name_lang vmangos
    // 22 string flags
